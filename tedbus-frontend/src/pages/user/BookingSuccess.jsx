...
const BookingSuccess = () => {
  ...
  const passengerData = Object.values(
    bookingData?.passenger?.passengers || {},
  )[0];
  ...
  const bookingId = "TED" + Math.floor(Math.random() * 1000000);
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        {/* Success Message */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-lg p-8 text-center">
          <FaCheckCircle className="mx-auto text-green-500" size={80} />

          <h1 className="text-4xl font-bold mt-4">Booking Confirmed 🎉</h1>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-lg p-8 mt-8">
          <div className="flex items-center gap-3 mb-6">
            <FaTicketAlt className="text-red-600" size={28} />
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="font-semibold text-lg">{passengerData?.name}</p>
            </div>

            <div>
              <p className="text-gray-500">Phone Number</p>

              <p className="font-semibold text-lg">{passengerData?.phone}</p>
            </div>

            <div>
              <p className="text-gray-500">Email</p>

              <p className="font-semibold text-lg">{passengerData?.email}</p>
            </div>

            <div>
              <p className="text-gray-500">Bus ID</p>

              <p className="font-semibold text-lg">{busId}</p>
            </div>

            <div>
              <p className="text-gray-500">Selected Seats</p>

              <p className="font-semibold text-lg">{seats.join(", ")}</p>
            </div>

            <div>
              <p className="text-gray-500">Total Amount</p>

              <p className="font-bold text-2xl text-red-600">₹{amount}</p>
            </div>
          </div>
        </div>
