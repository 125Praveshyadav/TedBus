  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Main Post Card */}
        <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-[2.5rem] border  border-slate-100 dark:border-slate-800 shadow-sm mb-6">
          {/* Author Info */}
          <Link
          ...
          >
            ...
            />
            <div>
              <h3 className="font-black text-slate-800 dark:text-slate-200 group-hover:text-red-600 dark:hover:text-red-400 transition-colors">
          ...
          {user && user._id === singlePost.author?._id && (
            <Link
              ...
              className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-2xl font-bold text-sm hover:bg-red-50 dark:bg-red-900/30 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 transition-all"
            >
              <Edit3 size={16} />
              Edit Post
            </Link>
          ...
          {singlePost.route?.source && singlePost.route?.destination && (
            <div className="flex items-center gap-1.5 text-sm font-bold text-red-500 mb-4 bg-red-50 dark:bg-red-900/30 w-fit px-3 py-1.5 rounded-xl">
              <MapPin size={14} />
              {singlePost.route.source} ➔ {singlePost.route.destination}
            </div>
          )}
          ...
          {/* Content */}
          <p className="text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-line mb-6">
            {singlePost.content}
          </p>
          ...
          {singlePost.tags?.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-6">
              {singlePost.tags.map((tag, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1 bg-slate-100 dark:bg-slate-800 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs font-bold rounded-xl"
                >
                  #{tag}
                </span>
              ))}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl font-bold transition-all active:scale-95 ${
                isLiked
                  ...
                  : "bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 hover:text-red-500"
              }`}
            >
              <Heart size={18} fill={isLiked ? "currentColor" : "none"} />
              {likeCount} Likes
            </button>
            <div className="flex items-center gap-2 px-5 py-2.5 rounded-2xl font-bold bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400">
